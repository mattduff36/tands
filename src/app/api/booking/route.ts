  ...
  try {
    ...
    try {
      booking = await createPendingBooking({
        ...
        startDate: validatedData.eventStartTime
          ? new Date(validatedData.eventStartTime)
          : undefined,
        endDate: validatedData.eventEndTime
          ? new Date(validatedData.eventEndTime)
          : undefined,
        eventDuration: validatedData.eventDuration,
      });
    } catch (dbError) {
      console.warn(
        "Database unavailable, creating mock booking for testing:",
        dbError,
      );
    ...
    try {
      await sendBookingReceivedEmail({
        ...
        endDate:
          validatedData.eventEndTime ||
          new Date(
            new Date(validatedData.eventDate).getTime() +
              validatedData.eventDuration * 60 * 60 * 1000,
          ).toISOString(),
        eventDuration: validatedData.eventDuration,
