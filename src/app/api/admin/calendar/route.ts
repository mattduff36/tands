...
export async function GET(request: NextRequest) {
  try {
    ...
    const now = new Date();
    const events = await calendarService.getEventsForMonth(now.getFullYear(), now.getMonth() + 1);
...
export async function POST(request: NextRequest) {
  try {
    ...
    switch (action) {
      ...
      case 'check-availability':
        ...
        if (!startDate || !endDate) {
          return NextResponse.json({ 
            error: 'startDate and endDate are required for availability check' 
          }, { status: 400 });
        }
      ...
      case 'get-available-slots':
        ...
        const slots = await calendarService.getAvailableTimeSlots(targetDate, duration);
        return NextResponse.json({
      ...
      case 'create-maintenance':
        ...
        if (isNaN(mStart.getTime()) || isNaN(mEnd.getTime())) {
          return NextResponse.json({ error: 'Invalid maintenance date format' }, { status: 400 });
        }
      ...
      case 'sync-calendar':
        ...
        const syncedEvents = await calendarService.getEventsInRange(syncStart, syncEnd);
    ...
    return NextResponse.json(
      { error: 'Failed to perform calendar operation', details: (error as Error).message },
      { status: 500 }
    );
  }
}