...
import { getCastles, updateCastleImageUrls, Castle } from '../src/lib/database/castles';
...
async function migrate() {
  ...
  const castles: Castle[] = await getCastles();
  const updates: { id: number; imageUrl: string }[] = [];
  const results: any[] = [];
